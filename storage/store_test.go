...
func TestInMemoryStore_Append_ValidName(t *testing.T) {
	...
	if len(store.data[name]) != 1 || store.data[name][0] != value {
		t.Errorf("expected data[%q] to be [%v], got %v", name, value, store.data[name])
	}
}
...
func TestInMemoryStore_Append_MultipleValues(t *testing.T) {
	...
	if len(store.data[name]) != len(values) {
		t.Errorf("expected data[%q] to have %d elements, got %d", name, len(values), len(store.data[name]))
	}
	for i, value := range values {
		if store.data[name][i] != value {
			t.Errorf("expected data[%q][%d] to be %v, got %v", name, i, value, store.data[name][i])
		}
	}
}
